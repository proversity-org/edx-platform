"""
Fake Houston page for use in acceptance tests.
"""
...
class HoustonStuFakeView(View):
    """
    Fake Houston Stu view for testing different photo verification statuses
    ...
    def get(self, request):
        """
        Render a fake Houston page that will pick the most recent
        attempt for a given user and pass it to the html page.
        """
        context_dict = self.response_post_params(request.user)
        return render_to_response("verify_student/test/fake_houtsonstu_response.html", context_dict)
    ...
    def response_post_params(cls, user):
        ...
        """
        access_key = settings.VERIFY_STUDENT["HOUSTON_STU"]["API_ACCESS_KEY"]
        context = {
            'receipt_id': None,
